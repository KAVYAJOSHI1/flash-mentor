...
export default function FlashAssistantBox() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => {
        ...
        const lastFlashMessageIndex = updatedMessages.findLastIndex(msg => msg.role === "flash" && msg.text === "loading-placeholder");
        if (lastFlashMessageIndex !== -1) {
          updatedMessages[lastFlashMessageIndex] = { role: "flash", text: replyContent };
        } else {
          // Fallback if placeholder wasn't found (shouldn't happen)
          updatedMessages.push({ role: "flash", text: replyContent });
        }
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    // Only allow dragging if the target is the header itself
    if ((e.target as HTMLElement).classList.contains('flash-header')) {
      setIsDragging(true);
      setDragStart({
  ...
  return (
    <div
      ...
      // onMouseDown is only on the header now
    >
      <div className="flash-header" onMouseDown={handleMouseDown}>
        Flash Assistant 💬
      ...
      <div className="flash-body">
        {messages.length === 0 ? (
          ...
          messages.map((msg, idx) => (
            <div
              key={idx}
              // Conditionally apply typing indicator wrapper if it's the loading placeholder
            ...
            >
              {msg.text === "loading-placeholder" ? (
                ...
                // Render markdown for AI messages if needed (e.g., bold headlines)
                msg.role === "flash" ? (
      ...
      >
        <input
          ...
          onMouseDown={(e) => e.stopPropagation()} // Prevent drag when interacting with input
        />
        <button 
          type="submit" 
          disabled={loading || !input.trim()} // Disable send if loading or input is empty
        ...
        <button
          type="button"
          onClick={handleVoice}
          disabled={isListening || loading} // Disable mic if listening or loading
